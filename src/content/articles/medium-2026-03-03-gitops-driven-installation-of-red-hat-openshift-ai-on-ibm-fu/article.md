---
title: "GitOps-Driven Installation of Red Hat OpenShift AI on IBM Fusion with Argo CD"
author: "Towards AI"
platform: "medium"
...
[Harichandana Kotha](https://medium.com/@harichandanakotha?source=post_page---byline--bca1c6776fd0---------------------------------------)
...
This grants full cluster privileges to the Argo CD controller. Use with caution. This ensures the Argo CD controller can reconcile all manifests defined in this guide.
...
```
fusion-openshift-ai/├── README.md                            # Documentation and deployment guide│├── gitops/│   ├── rhoai-application.yaml           # Argo CD Application CR│   ││   ├── patch/│   │   ├── argocd-patch-rbac.yaml       # RBAC to allow Argo CD CR patching│   │   └── argocd-resource-health-patch.yaml  # Argo CD health checks for RHOAI CRs│   ││   └── rhoai/│       ├── operator.yaml                # RHOAI Operator subscription and install│       ├── dsc.yaml                     # DataScienceCluster configuration│       └── kustomization.yaml           # Kustomize base
```
...
```
serviceaccount/argocd-patch-sa createdclusterrole.rbac.authorization.k8s.io/argocd-patch-clusterrole createdclusterrolebinding.rbac.authorization.k8s.io/argocd-patch-clusterrolebinding created
```

Next, patch the Argo CD Custom Resource to enable health checks for RHOAI components:

```
oc patch argocd openshift-gitops \  -n openshift-gitops \  --type merge \  --patch-file fusion-openshift-ai/gitops/patch/argocd-resource-health-patch.yaml
```
...
## Get Harichandana Kotha’s stories in your inbox
...
```
spec:  source:    repoURL: https://github.com/<your-username>/storage-fusion.git
```
...
```
source:  path: fusion-openshift-ai/gitops/rhoai
```
...
After applying the Argo CD Application, verify the installation from the OpenShift console.

Navigate to: **Red Hat Applications → OpenShift GitOps → Cluster Argo CD**
...
![rhoai-install application on ArgoCD dashboard]()
...
The installation begins with `operator.yaml`, which installs the RHOAI operator through OLM (Namespace, OperatorGroup, and Subscription). n this example, the operator installs from the fast-3.x update channel as defined in `operator.yaml`.
...
-   **Workbenches** — enable notebook environments created within user projects
...
Using this approach, Red Hat OpenShift AI is installed and managed declaratively on IBM Fusion HCI through a single Argo CD Application resource.
